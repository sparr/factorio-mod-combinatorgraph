...
#   test/ft/run.sh "wires"              # only tests matching a Lua pattern
#   test/ft/run.sh -g --no-auto-start    # open a window and pick tests by hand
#   test/ft/run.sh -g --game-speed 1     # open a window and watch
#
# CG_FACTORIO    the game binary, if it is not where Steam puts it here
# CG_FT_DATA     the throwaway data directory the run happens in
#
...
factorio="${CG_FACTORIO:-/home/sparr/Games/Steam/steamapps/common/Factorio/bin/x64/factorio}"
...
data="${CG_FT_DATA:-$HOME/.cache/cg-factorio-test}"
...
# cg-tests carries no prototypes; it is the gate that keeps the fixtures from registering
...
mkdir -p "$data/mods"
ln -sfn "$root/test/ft/cg-tests" "$data/mods/cg-tests"
